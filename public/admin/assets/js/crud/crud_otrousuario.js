...
var globalName = "OtroUsuario";


// TIPO DE USUARIO
function cargarTipoUsuario() {
    // var txt = $("input").val();
    console.log("RUTA : " + ruta() + "tipousuario" + globalName);

    $.post(ruta() + "tipousuario" + globalName, {cmd: 'web'}, function (json) {

        // SIRVE PARA SABER SI ESTA ACTIVO O INACTIVO
        var option_tipousuario = "<option value='' selected='selected'>Seleccionar Tipo de Usuario</option>";
        console.log("Probando :");
        try {
            if (json['data'] != null) {

                var tipousuario = json['data'];
                for (var c = 0; c < tipousuario.length; c++) {
                    option_tipousuario += "<option value='" + tipousuario[c].id + "' data-subtext=' (" + tipousuario[c].estado + ")'>" + tipousuario[c].nombre + "</option>";
                }
            }
        } catch (err) {
            //console.error(err);
        }
        $("#Rtipo").html(option_tipousuario);
        $('#Rtipo').selectpicker('refresh');
    });
}
cargarTipoUsuario();
